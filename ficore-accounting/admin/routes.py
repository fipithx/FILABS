...
class AgentManagementForm(FlaskForm):
    agent_id = StringField(t('agents_agent_id', default='Agent ID'), [
        DataRequired(message=t('agents_agent_id_required', default='Agent ID is required')),
        validators.Regexp(AGENT_ID_REGEX, message=t('agents_agent_id_format', default='Agent ID must be 8 alphanumeric characters'))
    ], render_kw={'class': 'form-control'})
    status = SelectField(t('agents_status', default='Status'), choices=[
        ('active', t('agents_active', default='Active')),
        ('inactive', t('agents_inactive', default='Inactive'))
    ], validators=[DataRequired(message=t('agents_status_required', default='Status is required'))], render_kw={'class': 'form-select'})
    submit = SubmitField(t('agents_manage_submit', default='Add/Update Agent'), render_kw={'class': 'btn btn-primary w-100'})

class NewsForm(FlaskForm):
    title = StringField(t('news_title', default='Title'), validators=[DataRequired()], render_kw={'class': 'form-control'})
    content = TextAreaField(t('news_content', default='Content'), validators=[DataRequired()], render_kw={'class': 'form-control'})
    source_link = StringField(t('news_source_link', default='Source Link'), render_kw={'class': 'form-control'})
    category = StringField(t('news_category', default='Category'), render_kw={'class': 'form-control'})
    submit = SubmitField(t('news_save', default='Save'), render_kw={'class': 'btn btn-primary'})

class TaxRateForm(FlaskForm):
    role = SelectField(t('tax_role', default='Role'), choices=[('personal', 'Personal'), ('trader', 'Trader'), ('agent', 'Agent'), ('company', 'Company'), ('vat', 'VAT')], validators=[DataRequired()], render_kw={'class': 'form-select'})
    min_income = FloatField(t('tax_min_income', default='Minimum Income'), validators=[DataRequired(), NumberRange(min=0)], render_kw={'class': 'form-control'})
    max_income = FloatField(t('tax_max_income', default='Maximum Income'), validators=[DataRequired(), NumberRange(min=0)], render_kw={'class': 'form-control'})
    rate = FloatField(t('tax_rate', default='Rate'), validators=[DataRequired(), NumberRange(min=0, max=1)], render_kw={'class': 'form-control'})
    description = StringField(t('tax_description', default='Description'), validators=[DataRequired()], render_kw={'class': 'form-control'})
    submit = SubmitField(t('tax_add_rate', default='Add Tax Rate'), render_kw={'class': 'btn btn-primary'})

    def validate_max_income(self, field):
        if field.data <= self.min_income.data:
            raise ValidationError(t('tax_max_income_error', default='Maximum income must be greater than minimum income.'))

class RoleForm(FlaskForm):
    role = SelectField(t('user_role', default='Role'), choices=[('personal', 'Personal'), ('trader', 'Trader'), ('agent', 'Agent'), ('admin', 'Admin')], validators=[DataRequired()], render_kw={'class': 'form-select'})
    submit = SubmitField(t('user_update_role', default='Update Role'), render_kw={'class': 'btn btn-primary'})

class PaymentLocationForm(FlaskForm):
    name = StringField(t('location_name', default='Location Name'), validators=[DataRequired(), validators.Length(min=2, max=100)], render_kw={'class': 'form-control'})
    address = StringField(t('location_address', default='Address'), validators=[DataRequired(), validators.Length(min=5, max=200)], render_kw={'class': 'form-control'})
    city = StringField(t('location_city', default='City'), validators=[DataRequired(), validators.Length(min=2, max=100)], render_kw={'class': 'form-control'})
    country = StringField(t('location_country', default='Country'), validators=[DataRequired(), validators.Length(min=2, max=100)], render_kw={'class': 'form-control'})
    submit = SubmitField(t('location_add', default='Add Payment Location'), render_kw={'class': 'btn btn-primary'})

class TaxDeadlineForm(FlaskForm):
    role = SelectField(t('tax_role', default='Role'), choices=[('personal', 'Personal'), ('trader', 'Trader'), ('agent', 'Agent'), ('company', 'Company'), ('vat', 'VAT')], validators=[DataRequired()], render_kw={'class': 'form-select'})
    deadline_date = DateField(t('tax_deadline_date', default='Deadline Date'), validators=[DataRequired()], format='%Y-%m-%d', render_kw={'class': 'form-control'})
    description = StringField(t('tax_description', default='Description'), validators=[DataRequired(), validators.Length(min=5, max=200)], render_kw={'class': 'form-control'})
    submit = SubmitField(t('tax_add_deadline', default='Add Tax Deadline'), render_kw={'class': 'btn btn-primary'})
...
def dashboard():
    ...
    try:
        ...
        return render_template(
            ...
            title=t('admin_dashboard_title', default='Admin Dashboard')
        )
    except Exception as e:
        logger.error(f"Error loading admin dashboard: {str(e)}")
        flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
...
def manage_users():
    ...
    try:
        ...
        for user in users:
            user['_id'] = str(user['_id'])
        return render_template('admin/users.html', users=users, title=t('admin_manage_users_title', default='Manage Users'))
    except Exception as e:
...
def suspend_user(user_id):
    ...
    try:
        ...
        if not user:
            flash(t('admin_user_not_found', default='User not found'), 'danger')
        ...
        if result.modified_count == 0:
            flash(t('admin_user_not_updated', default='User could not be suspended'), 'danger')
        else:
            flash(t('admin_user_suspended', default='User suspended successfully'), 'success')
...
def delete_user(user_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_user_not_deleted', default='User could not be deleted'), 'danger')
        else:
            flash(t('admin_user_deleted', default='User deleted successfully'), 'success')
...
def delete_item(collection, item_id):
    ...
    if collection not in valid_collections:
        flash(t('admin_invalid_collection', default='Invalid collection selected'), 'danger')
        return redirect(url_for('admin.dashboard'))
    try:
        db = utils.get_mongo_db()
        result = db[collection].delete_one({'_id': ObjectId(item_id)})
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Item not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Item deleted successfully'), 'success')
...
def view_credit_requests():
    ...
    try:
        ...
        return render_template(
            ...
            requests=requests,
            title=t('credits_requests_title', default='Pending Credit Requests')
        )
    except Exception as e:
        logger.error(f"Error fetching credit requests for admin {current_user.id}: {str(e)}")
        flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
        return render_template('admin/credits_requests.html', requests=[], title=t('general_error', default='Error'))
...
def manage_credit_request(request_id):
    ...
    try:
        ...
        if not request_data:
            flash(t('credits_request_not_found', default='Credit request not found'), 'danger')
        ...
        if form.validate_on_submit():
            ...
            flash(t(f'credits_request_{status}', default=f'Credit request {status} successfully'), 'success')
        ...
        return render_template(
            ...
            title=t('credits_manage_request_title', default='Manage Credit Request')
        )
    except Exception as e:
...
def audit():
    ...
    try:
        ...
        for log in logs:
            log['_id'] = str(log['_id'])
        return render_template('admin/audit.html', logs=logs, title=t('admin_audit_title', default='Audit Logs'))
    except Exception as e:
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            if existing_agent:
                ...
                if result.modified_count == 0:
                    flash(t('agents_not_updated', default='Agent status could not be updated'), 'danger')
                else:
                    flash(t('agents_status_updated', default='Agent status updated successfully'), 'success')
                ...
                flash(t('agents_added', default='Agent ID added successfully'), 'success')
        ...
        return render_template('admin/manage_agents.html', form=form, agents=agents, title=t('admin_manage_agents_title', default='Manage Agents'))
...
def admin_budgets():
    ...
    try:
        ...
        return render_template('admin/budgets.html', budgets=budgets, title=t('admin_budgets_title', default='Manage Budgets'))
    except Exception as e:
...
def admin_delete_budget(budget_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Budget not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Budget deleted successfully'), 'success')
...
def admin_bills():
    ...
    try:
        ...
        return render_template('admin/bills.html', bills=bills, title=t('admin_bills_title', default='Manage Bills'))
    except Exception as e:
...
def admin_delete_bill(bill_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Bill not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Bill deleted successfully'), 'success')
...
def admin_mark_bill_paid(bill_id):
    ...
    try:
        ...
        if result.modified_count == 0:
            flash(t('admin_item_not_updated', default='Bill could not be updated'), 'danger')
        else:
            flash(t('admin_bill_marked_paid', default='Bill marked as paid'), 'success')
...
def admin_emergency_funds():
    ...
    try:
        ...
        return render_template('admin/emergency_funds.html', funds=funds, title=t('admin_emergency_funds_title', default='Manage Emergency Funds'))
    except Exception as e:
...
def admin_delete_emergency_fund(fund_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Emergency fund not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Emergency fund deleted successfully'), 'success')
...
def admin_net_worth():
    ...
    try:
        ...
        for nw in net_worths:
            nw['_id'] = str(nw['_id'])
        return render_template('admin/net_worth.html', net_worths=net_worths, title=t('admin_net_worth_title', default='Manage Net Worth'))
    except Exception as e:
...
def admin_delete_net_worth(nw_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Net worth record not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Net worth record deleted successfully'), 'success')
...
def admin_quiz_results():
    ...
    try:
        ...
        return render_template('admin/quiz_results.html', quiz_results=quiz_results, title=t('admin_quiz_results_title', default='Manage Quiz Results'))
    except Exception as e:
...
def admin_delete_quiz_result(result_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Quiz result not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Quiz result deleted successfully'), 'success')
    ...
    try:
        ...
        if request.method == 'POST' and form.validate_on_submit():
            if not allowed_file(form.file.data.filename):
                flash(t('learning_hub_invalid_file_type', default='Invalid file type. Allowed: mp4, pdf, txt, md'), 'danger')
                return render_template('admin/learning_hub.html', form=form, courses=courses, progress=[], title=t('admin_manage_courses_title', default='Manage Courses'))
            ...
            flash(t('learning_hub_upload_success', default='Content uploaded successfully'), 'success')
...
def delete_course(course_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Course not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Course deleted successfully'), 'success')
...
def admin_learning_hub():
    ...
    try:
        ...
        for p in progress:
            p['_id'] = str(p['_id'])
        form = UploadForm()
        return render_template('admin/learning_hub.html', form=form, progress=progress, courses=[], title=t('admin_learning_hub_title', default='Manage Learning Hub'))
    except Exception as e:
...
def admin_delete_learning_progress(progress_id):
    ...
    try:
        ...
        if result.deleted_count == 0:
            flash(t('admin_item_not_found', default='Learning progress not found'), 'danger')
        else:
            flash(t('admin_item_deleted', default='Learning progress deleted successfully'), 'success')
...
def news_management():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        flash(t('news_article_added', default='News article added successfully'), 'success')
    ...
    return render_template('admin/news_management.html', form=form, articles=articles, title=t('admin_news_management_title', default='News Management'))
...
def edit_news(article_id):
    ...
    if not article:
        flash(t('news_article_not_found', default='Article not found'), 'danger')
        return redirect(url_for('admin.news_management'))
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        flash(t('news_article_updated', default='News article updated successfully'), 'success')
        return redirect(url_for('admin.news_management'))
    
    return render_template('admin/news_edit.html', form=form, article=article, title=t('admin_edit_news_title', default='Edit News Article'))
...
def delete_news(article_id):
    ...
    if result.deleted_count > 0:
        ...
        flash(t('news_article_deleted', default='News article deleted successfully'), 'success')
    else:
        flash(t('news_article_not_found', default='Article not found'), 'danger')
    return redirect(url_for('admin.news_management'))
...
def manage_tax_rates():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('tax_rate_added', default='Tax rate added successfully'), 'success')
    ...
    return render_template('admin/tax_rates.html', form=form, rates=rates, title=t('admin_tax_rates_title', default='Manage Tax Rates'))
...
def edit_tax_rate(rate_id):
    ...
    if not rate:
        flash(t('tax_rate_not_found', default='Tax rate not found'), 'danger')
        return redirect(url_for('admin.manage_tax_rates'))
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('tax_rate_updated', default='Tax rate updated successfully'), 'success')
            return redirect(url_for('admin.manage_tax_rates'))
        except Exception as e:
            logger.error(f"Error updating tax rate {rate_id}: {str(e)}")
            flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
            return render_template('admin/tax_rate_edit.html', form=form, rate=rate, title=t('admin_edit_tax_rate_title', default='Edit Tax Rate'))
...
def delete_tax_rate(rate_id):
    ...
    if result.deleted_count > 0:
        ...
        flash(t('tax_rate_deleted', default='Tax rate deleted successfully'), 'success')
    else:
        flash(t('tax_rate_not_found', default='Tax rate not found'), 'danger')
    return redirect(url_for('admin.manage_tax_rates'))
...
def manage_payment_locations():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('payment_location_added', default='Payment location added successfully'), 'success')
    ...
    return render_template('admin/payment_locations.html', form=form, locations=locations, title=t('admin_payment_locations_title', default='Manage Payment Locations'))
...
def edit_payment_location(location_id):
    ...
    if not location:
        flash(t('payment_location_not_found', default='Payment location not found'), 'danger')
        return redirect(url_for('admin.manage_payment_locations'))
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('payment_location_updated', default='Payment location updated successfully'), 'success')
            return redirect(url_for('admin.manage_payment_locations'))
        except Exception as e:
            logger.error(f"Error updating payment location {location_id}: {str(e)}")
            flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
            return render_template('admin/payment_location_edit.html', form=form, location=location, title=t('admin_edit_payment_location_title', default='Edit Payment Location'))
...
def delete_payment_location(location_id):
    ...
    if result.deleted_count > 0:
        ...
        flash(t('payment_location_deleted', default='Payment location deleted successfully'), 'success')
    else:
        flash(t('payment_location_not_found', default='Payment location not found'), 'danger')
    return redirect(url_for('admin.manage_payment_locations'))
...
def manage_tax_deadlines():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('tax_deadline_added', default='Tax deadline added successfully'), 'success')
    ...
    return render_template('admin/tax_deadlines.html', form=form, deadlines=deadlines, title=t('admin_tax_deadlines_title', default='Manage Tax Deadlines'))
...
def edit_tax_deadline(deadline_id):
    ...
    if not deadline:
        flash(t('tax_deadline_not_found', default='Tax deadline not found'), 'danger')
        return redirect(url_for('admin.manage_tax_deadlines'))
    ...
    if request.method == 'POST' and form.validate_on_submit():
        try:
            ...
            flash(t('tax_deadline_updated', default='Tax deadline updated successfully'), 'success')
            return redirect(url_for('admin.manage_tax_deadlines'))
        except Exception as e:
            logger.error(f"Error updating tax deadline {deadline_id}: {str(e)}")
            flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
            return render_template('admin/tax_deadline_edit.html', form=form, deadline=deadline, title=t('admin_edit_tax_deadline_title', default='Edit Tax Deadline'))
...
def delete_tax_deadline(deadline_id):
    ...
    if result.deleted_count > 0:
        ...
        flash(t('tax_deadline_deleted', default='Tax deadline deleted successfully'), 'success')
    else:
        flash(t('tax_deadline_not_found', default='Tax deadline not found'), 'danger')
    return redirect(url_for('admin.manage_tax_deadlines'))
...
def customer_reports():
    ...
    return render_template('admin/customer_reports.html', users=users, title=t('admin_customer_reports_title', default='Customer Reports'))
...
def generate_customer_report_pdf(users):
    ...
    p.drawString(1 * inch, 10.5 * inch, t('admin_customer_report_title', default='Customer Report'))
    p.drawString(1 * inch, 10.2 * inch, f"{t('admin_generated_on', default='Generated on')}: {datetime.datetime.utcnow().strftime('%Y-%m-%d')}")
    y = 9.5 * inch
    p.drawString(1 * inch, y, t('admin_username', default='Username'))
    p.drawString(2.5 * inch, y, t('admin_email', default='Email'))
    p.drawString(4 * inch, y, t('user_role', default='Role'))
    p.drawString(5.5 * inch, y, t('admin_created_at', default='Created At'))
    y -= 0.3 * inch
    for user in users:
...
def generate_customer_report_csv(users):
    ...
    output = [[t('admin_username', default='Username'), t('admin_email', default='Email'), t('user_role', default='Role'), t('admin_created_at', default='Created At')]]
    for user in users:
...
def manage_user_roles():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if not user_id:
            flash(t('user_id_required', default='User ID is required'), 'danger')
            return redirect(url_for('admin.manage_user_roles'))
        try:
            user = db.users.find_one({'_id': ObjectId(user_id)})
            if not user:
                flash(t('user_not_found', default='User not found'), 'danger')
                return redirect(url_for('admin.manage_user_roles'))
            ...
            flash(t('user_role_updated', default='User role updated successfully'), 'success')
            return redirect(url_for('admin.manage_user_roles'))
        except Exception as e:
            logger.error(f"Error updating user role {user_id}: {str(e)}")
            flash(t('admin_database_error', default='An error occurred while accessing the database'), 'danger')
            return render_template('admin/user_roles.html', form=form, users=users, title=t('admin_manage_user_roles_title', default='Manage User Roles'))
